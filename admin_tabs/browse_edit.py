...
def show_browse_edit_tab():
    ...
    try:
        ...
        if not training_examples:
            st.info("No training examples found. Add some using the 'Add New' tab or batch import script.")
        else:
            ...
            # Genre filter
            if genre_filter != "All":
                filtered_examples = [ex for ex in filtered_examples if ex["genre"] == genre_filter]
            ...
            if search_query.strip():
                ...
                filtered_examples = [
                    ...
                    if (search_lower in ex['input_track']['file_path'].lower() or
                        search_lower in ex['reference_track']['file_path'].lower())
                ]
            ...
            st.markdown(f"**Showing {len(filtered_examples)} of {len(training_examples)} examples**")

            if len(filtered_examples) == 0:
                st.info("No examples match your filters. Try adjusting the search criteria.")
            ...
            for i, example in enumerate(filtered_examples):
                with st.expander(
                    ...
                    with col1:
                        ...
                        st.text(f"File: {Path(example['input_track']['file_path']).name}")
                    ...
                    with col2:
                        ...
                        st.text(f"File: {Path(example['reference_track']['file_path']).name}")
                        st.text(f"Duration: {example['reference_track']['duration']:.1f}s")
                    ...
                    has_placeholder = any("[EDIT ME]" in fb['text'] for fb in example['feedback_items'])
                    if has_placeholder:
                        st.warning("⚠️ This entry has placeholder feedback that needs editing!")
                    ...
                    feedback_updates = []
                    feedback_types = ["general", "rhythm", "rhythm_practical", "eq", "eq_practical"]
                    ...
                    for j, feedback in enumerate(example['feedback_items']):
                        with st.container():
                            ...
                            if not delete_fb:  # Only include if not marked for deletion
                                ...
                                st.success("✅ This feedback will be deleted when you save changes")

                            st.markdown("---")

                    # Add new feedback option
                    st.markdown("**Add New Feedback:**")
                    add_new = st.checkbox(f"Add new feedback item", key=f"add_new_{example['id']}")

                    if add_new:
                        new_fb_type = st.selectbox(
                            "New feedback type:",
                            ["general", "rhythm", "rhythm_practical", "eq", "eq_practical"],
                    ...
                    # Save changes button
                    if st.button(f"Save Changes", key=f"save_{example['id']}", type="primary"):
                        try:
                            genre_to_update = new_genre if new_genre != current_genre else None
