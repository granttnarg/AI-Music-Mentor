...
def show_similarity_viz_tab():
    ...
    st.caption("Upload a track to see how it relates to all tracks in your database in 3D embedding space")
    ...
    if uploaded_file:
        ...
        with col1:
            ...
            similarity_metric = st.selectbox("Similarity Metric", ["cosine", "euclidean", "inner_product"])
        with col2:
            reduction_method = st.selectbox("Dimensionality Reduction", ["PCA", "t-SNE"])
            training_only = st.checkbox("Only search tracks with training examples", value=False,
        ...
        if st.button("Generate 3D Similarity Visualization", type="primary"):
            with st.spinner("Processing audio and generating visualization..."):
                try:
                    ...
                    global_features = service.load_audio_file(temp_path).extract_global_features(max_duration=400)
                    ...
                    try:
                        ...
                        all_tracks = session.query(Track).filter(Track.global_embedding.isnot(None)).all()
                        ...
                        if training_only:
                            ...
                            if len(search_tracks) < 1:
                                st.error(f"No tracks with training examples found for similarity search. Found: {len(search_tracks)}")
                                return
                        else:
                            search_tracks = all_tracks

                        if len(all_tracks) < 3:
                            st.error(f"Need at least 3 tracks with embeddings in database for visualization. Found: {len(all_tracks)}")
                            return
                        ...
                        track_info.append({
                            ...
                            'id': -1,
                            'type': 'query',
                            'duration': len(global_features.get('tempo', [0])) * 0.1  # rough estimate
                        })
                        ...
                        if training_only:
                            similar_tracks = db_ops.find_similar_tracks_with_training_examples(
                        ...
                        try:
                            ...
                            if similarity_metric == "cosine":
                                distance_col = Track.global_embedding.cosine_distance(list(query_embedding))
                                base_query = session_for_distances.query(Track, distance_col.label('distance'))
                                order_by = distance_col
                            elif similarity_metric == "euclidean":
                                distance_col = Track.global_embedding.l2_distance(list(query_embedding))
                                base_query = session_for_distances.query(Track, distance_col.label('distance'))
                                order_by = distance_col
                            elif similarity_metric == "inner_product":
                                score_col = Track.global_embedding.max_inner_product(list(query_embedding))
                                base_query = session_for_distances.query(Track, score_col.label('distance'))
                            ...
                            if base_query is not None and order_by is not None:
                                results_with_distances = base_query.order_by(order_by).limit(k_similar * 2).all()  # Get more to handle duplicates

                                # Remove duplicates manually and limit results
                                seen_ids = set()
                                unique_results = []
                                for row in results_with_distances:
                                    if row[0].id not in seen_ids and len(unique_results) < k_similar:
                                        unique_results.append(row)
                                        seen_ids.add(row[0].id)

                                # Extract tracks and their actual distance/similarity values
                                similar_tracks_with_scores = [(row[0], float(row[1])) for row in unique_results]
                            else:
                                similar_tracks_with_scores = [(track, 0.0) for track in similar_tracks[:k_similar]]
                        ...
                        for track in all_tracks:
                            if track.id in similar_track_data:
                                ...
                                similarities.append(0.1)  # Low similarity for non-matches
                        ...
                        if reduction_method == "PCA":
                            ...
                            reducer = TSNE(n_components=3, random_state=42, perplexity=min(30, len(embeddings)//2))
                        ...
                        for i, (coord, info) in enumerate(zip(coords_3d, track_info)):
                            if info['type'] == 'query':
                                ...
                                if rank_in_similar == 0:
                                    color = 'rgba(0, 255, 0, 0.9)'  # Bright green for #1
                            ...
                            viz_data.append({
                                ...
                                'size': size,
                                'symbol': symbol,
                                'similarity': similarities[i] if i < len(similarities) else 0.1,
                                'type': info['type']
                            })
                        ...
                        if not query_data.empty:
                            fig.add_trace(go.Scatter3d(
                                x=query_data['x'], y=query_data['y'], z=query_data['z'],
                                mode='markers',
                        ...
                        if not similar_data.empty:
                            fig.add_trace(go.Scatter3d(
                                x=similar_data['x'], y=similar_data['y'], z=similar_data['z'],
                                mode='markers',
                                marker=dict(
                        ...
                        # Add other tracks
                        other_indices = [i for i in range(len(df)-1) if i not in top_indices]  # -1 to exclude query
                        other_data = df[df.index.isin(other_indices)]
                        if not other_data.empty:
                            fig.add_trace(go.Scatter3d(
                                x=other_data['x'], y=other_data['y'], z=other_data['z'],
                                mode='markers',
                                marker=dict(size=5, color='lightblue', opacity=0.4, symbol='circle'),
                        ...
                        title_text = f"🎵 3D Audio Similarity Space ({similarity_metric} metric)"
                        if training_only:
                            title_text += " - Training Tracks Only"
                        title_text += f"<br><sup>{axis_explanation[reduction_method]}</sup>"
                        ...
                        method_display = "Training Tracks Only" if training_only else "All Tracks"
                        st.subheader(f"Top Similar Tracks ({method_display} - {similarity_metric} metric)")
                        ...
                        if similarity_metric == "cosine":
                            st.caption("📊 **Cosine Distance**: 0.0 = identical, 2.0 = completely opposite. Lower values = more similar.")
                        elif similarity_metric == "euclidean":
                            st.caption("📊 **Euclidean Distance**: 0.0 = identical, higher values = more different. Lower values = more similar.")
                        elif similarity_metric == "inner_product":
                            st.caption("📊 **Inner Product**: Higher positive values = more similar. Measures alignment between embeddings.")
    ...
    try:
        ...
        try:
            ...
            tracks_with_embeddings = session.query(Track).filter(Track.global_embedding.isnot(None)).count()
