...
def show_admin_eval_tab():
    ...
    st.caption("Systematically evaluate LLM output quality to track improvements over time")
    ...
    def get_latest_upload_info():
        try:
            ...
            latest_upload = session.query(UserUpload).order_by(UserUpload.id.desc()).first()

            if latest_upload:
                # Get track info
                input_track = session.query(Track).filter(Track.id == latest_upload.input_track_id).first()
                ref_track = session.query(Track).filter(Track.id == latest_upload.reference_track_id).first()

                return {
                    'upload_id': latest_upload.id,
                    'input_filename': Path(input_track.file_path).name if input_track else "",
                    'reference_filename': Path(ref_track.file_path).name if ref_track else "",
                    'session_id': latest_upload.session_id
                }
        except Exception as e:
            print(f"Error getting latest upload: {e}")
            return {'upload_id': None, 'input_filename': "", 'reference_filename': "", 'session_id': None}
        finally:
    ...
    if latest_info['upload_id']:
        st.caption(f"💡 Pre-filled with latest upload info (ID: {latest_info['upload_id']}) - edit as needed")
    ...
    with st.form("evaluation_form"):
        ...
        default_track_id = f"upload_{latest_info['upload_id']}" if latest_info['upload_id'] else ""
        ...
        with st.expander("📝 Paste Full RAG Prompt (helpful for tracking prompt changes)"):
            st.caption("This helps correlate prompt modifications with output quality changes")
        ...
        **What it measures:** How well the system describes what's actually happening in the audio
        ...
        """)
        audio_description = st.selectbox("Audio Description Accuracy", [1, 2, 3, 4, 5], key="audio_desc")
        ...
        **What it measures:** How actionable and useful the feedback is for improvement
        ...
        """)
        practical_advice = st.selectbox("Practical Advice Quality", [1, 2, 3, 4, 5], key="practical")
        ...
        **What it measures:** How well the feedback addresses the specific track analyzed
        ...
        """)
        relevance = st.selectbox("Relevance to Input Audio", [1, 2, 3, 4, 5], key="relevance")
        ...
        **What it measures:** How truthful the system is, avoiding false or made-up information
        ...
        """)
        truthfulness = st.selectbox("Truthfulness (No Hallucinations)", [1, 2, 3, 4, 5], key="truth")
        ...
        **What it measures:** Demonstrates understanding of arrangement patterns and audio features
        ...
        """)
        technical = st.selectbox("Technical Understanding", [1, 2, 3, 4, 5], key="technical")
        ...
        if submitted:
            if not llm_output.strip():
                ...
                clean_input_name = "".join(c for c in input_filename.strip() if c.isalnum() or c in ('-', '_'))[:30]

                evaluation = {
                    'id': eval_id,
                    'timestamp': datetime.now().isoformat(),
                    'input_filename': input_filename.strip(),
                    'reference_filename': reference_filename.strip() if reference_filename else None,
                ...
                eval_filename = f"eval_{eval_id:03d}_{timestamp_str}_{clean_input_name}.json"
                ...
                try:
                    ...
                    st.success(f"✅ Evaluation saved to `{eval_filename}`! Score: {weighted_score}/{max_score} ({percentage}%)")

                    # Show score breakdown
                    st.markdown("### Score Breakdown:")
                    st.markdown(f"- Audio Description Accuracy: {audio_description}/5 (×{weights['audio_description']}) = {audio_description * weights['audio_description']}")
                    st.markdown(f"- Practical Advice Quality: {practical_advice}/5 (×{weights['practical_advice']}) = {practical_advice * weights['practical_advice']}")
                    st.markdown(f"- Relevance to Input Audio: {relevance}/5 (×{weights['relevance']}) = {relevance * weights['relevance']}")
                    st.markdown(f"- Truthfulness: {truthfulness}/5 (×{weights['truthfulness']}) = {truthfulness * weights['truthfulness']}")
                    st.markdown(f"- Technical Understanding: {technical}/5 (×{weights['technical']}) = {technical * weights['technical']}")
                    st.markdown(f"**Total: {weighted_score}/{max_score} ({percentage}%)**")
    ...
    if evaluations:
        ...
        st.metric("Average Score", f"{avg_score:.1f}%", f"{len(evaluations)} evaluations")
        ...
        if len(evaluations) >= 2:
            ...
            ax.plot(df['evaluation_number'], df['percentage'], 'o-', color='#1f77b4', linewidth=2, markersize=4)
            ...
            if len(evaluations) >= 3:
                ...
                p = np.poly1d(z)
                ax.plot(df['evaluation_number'], p(df['evaluation_number']), "--", color='red', alpha=0.7, label=f'Trend (slope: {z[0]:.1f}%/eval)')
                ax.legend()
        ...
        for eval_data in reversed(evaluations[-10:]):  # Show last 10
            ...
            title_id = eval_data.get('input_filename', eval_data.get('track_id', 'Unknown'))
            with st.expander(f"ID {eval_data['id']} - {title_id} - {eval_data['percentage']}% ({eval_data['date']})"):
                ...
                with col1:
                    ...
                    st.markdown(f"**Weighted Total: {eval_data['weighted_score']}/{eval_data['max_score']} ({eval_data['percentage']}%)**")

                with col2:
                    st.markdown("**Details:**")
                    st.markdown(f"**Input File:** {eval_data.get('input_filename', 'N/A')}")
                    if eval_data.get('reference_filename'):
                        st.markdown(f"**Reference File:** {eval_data['reference_filename']}")
                ...
                with col_btn1:
                    if st.button(f"Show LLM Output", key=f"show_{eval_data['id']}"):
                        st.text_area("LLM Output:", eval_data['llm_output'], height=200, key=f"output_{eval_data['id']}")

                with col_btn2:
                    if eval_data.get('full_prompt') and st.button(f"Show Prompt", key=f"show_prompt_{eval_data['id']}"):
                        st.text_area("Full Prompt Used:", eval_data['full_prompt'], height=300, key=f"prompt_{eval_data['id']}")
                ...
                with col_btn3:
                    ...
                    if eval_data.get('full_prompt') and prev_eval_with_prompt:
                        if st.button(f"Diff vs #{prev_eval_with_prompt['id']}", key=f"diff_{eval_data['id']}"):
                            st.markdown(f"**Prompt Diff: #{prev_eval_with_prompt['id']} → #{eval_data['id']}**")

                            # Create unified diff
                            prev_lines = prev_eval_with_prompt['full_prompt'].splitlines(keepends=True)
                            curr_lines = eval_data['full_prompt'].splitlines(keepends=True)
        ...
        if st.button("📊 Export All Evaluations"):
            ...
            export_data = {
                ...
                "average_score": sum(e['percentage'] for e in evaluations) / len(evaluations) if evaluations else 0,
        ...
        st.info("No evaluations recorded yet. Complete the form above to start tracking feedback quality!")