...
class SongVisualizerService:
    ...
    def plot_time_aligned_comparison(
        ...
        for i, block in enumerate(input_arrangement_blocks):
            ...
            end_time = min(
                block["end_time"], input_duration
            )  # Clip to actual track duration
            section = block["arrangement_section"]

            print(
                f"DEBUG VIZ: Block {i+1}: {section} | {start_time:.2f}-{original_end_time:.2f}s (clipped to {end_time:.2f}s)"
            )

            # Skip blocks that start after the track ends
            if start_time >= input_duration:
                print(
                    f"DEBUG VIZ: SKIPPING block {section} - starts after track ends ({start_time:.2f} >= {input_duration:.2f})"
                )
                continue
        ...
        if input_duration < max_duration:
            ...
            ax1.text(
                input_duration + (max_duration - input_duration) / 2,
                0,
                "Track Ends",
                ha="center",
                va="center",
        ...
        for block in reference_arrangement_blocks:
            start_time = block["start_time"]
            end_time = min(
                block["end_time"], reference_duration
            )  # Clip to actual track duration
            section = block["arrangement_section"]
        ...
        if reference_duration < max_duration:
            ...
            ax2.text(
                reference_duration + (max_duration - reference_duration) / 2,
                0,
                "Track Ends",
                ha="center",
                va="center",
