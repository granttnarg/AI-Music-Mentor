...
class TestAudioFeatureService:
    ...
    def test_global_feature_extraction_structure(
        self, mock_exists, mock_load, service, mock_audio_data
    ):
        ...
        proportions = (
            frequency["low_proportion"]
            + frequency["mid_proportion"]
            + frequency["high_proportion"]
        )
        assert (
            0.95 < proportions < 1.05
        ), f"Frequency proportions should sum to ~1, got {proportions}"

    @patch("librosa.load")
    @patch("os.path.exists")
    def test_embedding_vector_shape_and_type(
        self, mock_exists, mock_load, service, mock_audio_data
    ):
        ...
        assert vector.dtype in [
            np.float64,
            np.float32,
        ], f"Expected float type, got {vector.dtype}"

        # Test values are finite and reasonable
        assert np.all(np.isfinite(vector)), "Vector contains non-finite values"
        assert np.all(
            vector >= 0
        ), "Vector contains negative values (should be normalized to 0-1+ range)"
