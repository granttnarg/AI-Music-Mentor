...
def backup_training_data(db_connection_url: str, backup_dir: Path = None):
    ...
    try:
        ...
        for example in training_examples:
            ...
            example_track = session.query(Track).filter_by(id=example.example_track_id).first()
            reference_track = session.query(Track).filter_by(id=example.reference_track_id).first()
            
            # Get feedback items
            feedback_items = session.query(Feedback).filter_by(training_example_id=example.id).all()
            ...
            example_data = {
                ...
                "example_track": {
                    ...
                    "global_embedding": convert_numpy_types(example_track.global_embedding) if example_track.global_embedding is not None else None,
                    "global_feature_data": convert_numpy_types(example_track.global_feature_data) if example_track.global_feature_data is not None else None,
                    ...
                    "viz_generated_at": example_track.viz_generated_at.isoformat() if example_track.viz_generated_at else None,
                } if example_track else None,
                "reference_track": {
                    ...
                    "global_embedding": convert_numpy_types(reference_track.global_embedding) if reference_track.global_embedding is not None else None,
                    "global_feature_data": convert_numpy_types(reference_track.global_feature_data) if reference_track.global_feature_data is not None else None,
                    ...
                    "viz_generated_at": reference_track.viz_generated_at.isoformat() if reference_track.viz_generated_at else None,
        ...
        metadata = {
            ...
            "total_feedback_items": sum(len(ex["feedback_items"]) for ex in training_data),
            "db_connection_url": db_connection_url.replace(os.getenv("DB_PASSWORD", ""), "***") if "DB_PASSWORD" in str(db_connection_url) else db_connection_url,
        }
