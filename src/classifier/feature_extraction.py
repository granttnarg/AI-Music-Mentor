...
class AudioFeature:
    ...
    def _create_meter_grid(self, target_segment_seconds=5) -> np.ndarray:
        ...
        beats_per_segment = float(beats_per_segment) if hasattr(beats_per_segment, '__len__') else beats_per_segment

        # Round to nearest multiple of time_signature for musical alignment
        beats_per_segment = max(self.time_signature,
        ...
        segment_times_forward = np.arange(first_beat_time, time_duration, segment_duration)
        segment_times_backward = np.arange(first_beat_time - segment_duration, -segment_duration, -segment_duration)

        # Create segment grid in both directions
        segment_grid = np.concatenate((np.array([0.0]), segment_times_backward[::-1], segment_times_forward))
        ...
        # Convert to frames
        meter_grid_frames = librosa.time_to_frames(segment_grid, sr=self.sr, hop_length=self.hop_length)
        ...
        segment_lengths = np.diff(librosa.frames_to_time(meter_grid_frames, sr=self.sr, hop_length=self.hop_length))
        print(f"Segment lengths: min={segment_lengths.min():.1f}s, max={segment_lengths.max():.1f}s, mean={segment_lengths.mean():.1f}s")
...
class AudioMeter:
  ...
  def segment_data_meters(self, data: np.ndarray, meter_grid: List[int]) -> List[np.ndarray]:
  ...
  def apply_hierarchical_positional_encoding(self, segments: List[np.ndarray]) -> List[np.ndarray]:
    ...
    for meter_idx, meter_segment in enumerate(segments):
        ...
        for frame_idx, frame in enumerate(meter_segment):
            frame_pos_encoding = self.positional_encoding(frame_idx, frame.shape[0]) * 0.1
            meter_pos_encoding = self.positional_encoding(meter_idx, frame.shape[0]) * 0.2
...
class AudioProcess:
    ...
    def pad_song(self, encoded_segments: List[np.ndarray], max_frames: int = MAX_FRAMES,
    ...
    def process_audio(self, audio_path, trim_silence=True, sr=SR, hop_length=HOP_LENGTH):
      ...
      try:
          ...
          feature_segments = audio_meter.segment_data_meters(audio_features.combined_features, meter_grid)
          encoded_segments = audio_meter.apply_hierarchical_positional_encoding(feature_segments)
